...
export default function MarketingLayout({
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white relative">
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm' : 'bg-transparent border-b border-white/10'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
             <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${scrolled ? 'bg-slate-100 text-slate-900' : 'bg-white/10 text-white'}`}>
                <RetinaLogo className="w-5 h-5" />
             </div>
             <span className={`font-bold text-xl tracking-tight transition-colors ${scrolled ? 'text-slate-900' : 'text-white'}`}>RETINA. Wealth</span>
          </div>

          <nav className={`hidden md:flex items-center gap-8 text-sm font-medium transition-colors ${scrolled ? 'text-slate-500' : 'text-slate-300'}`}>
             <Link href="#logic" className={`transition-colors ${scrolled ? 'hover:text-slate-900' : 'hover:text-white'}`}>How it Works</Link>
             <Link href="#performance" className={`transition-colors ${scrolled ? 'hover:text-slate-900' : 'hover:text-white'}`}>Performance</Link>
             <Link href="#pricing" className={`transition-colors ${scrolled ? 'hover:text-slate-900' : 'hover:text-white'}`}>Pricing</Link>
          </nav>

          <div className="flex items-center gap-4">
            <Link href="/dashboard" className={`text-sm font-medium transition-colors ${scrolled ? 'text-slate-600 hover:text-slate-900' : 'text-slate-300 hover:text-white'}`}>Log in</Link>
            <Link href="/dashboard" className={`text-sm font-bold px-5 py-2.5 rounded-full transition-colors shadow-sm ${scrolled ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-emerald-600 text-white hover:bg-emerald-500'}`}>
              Get Access
            </Link>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-8 text-center md:text-left md:col-span-2">
              <div>
                ...
                <ul className="space-y-2.5 text-sm text-slate-400">
                  ...
                  <li><Link href="/signup" className="hover:text-white transition-colors">Get Access</Link></li>
